...
public class SheyingshiController {
    ...
    */
    ...
    //    	ValidatorUtils.validateEntity(user);
        if(sheyingshiService.selectOne(new EntityWrapper<SheyingshiEntity>().eq("username", sheyingshi.getUsername()).orNew().eq("sheyingshi_phone",sheyingshi.getSheyingshiPhone()).orNew().eq("sheyingshi_id_number",sheyingshi.getSheyingshiIdNumber())) !=null) {
            return R.error("账户已存在或手机号或身份证号已经被使用");
        }
        sheyingshi.setSheyingshiContent("");
        sheyingshi.setSheyingshiXingjiTypes(1);
        sheyingshi.setCreateTime(new Date());
        sheyingshiService.insert(sheyingshi);
        return R.ok();
    }
