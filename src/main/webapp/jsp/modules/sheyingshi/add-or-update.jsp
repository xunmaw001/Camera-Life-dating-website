            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑摄影师</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">摄影师信息</h3>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("sheyingshi/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addsheyingshi');
    ...
    function setSelectOption() {
        ...
        if(fenggeTypesSelect != null && fenggeTypesOptions != null  && fenggeTypesOptions.length > 0 ) {
            for (var i = 0; i < fenggeTypesOptions.length; i++) {
                if (fenggeTypesOptions[i].codeIndex == ruleForm.fenggeTypes) {//下拉框value对比,如果一致就赋值汉字
                        fenggeTypesSelect.options[i].selected = true;
                }
            }
        }
        ...
        if(sheyingshiXingjiTypesSelect != null && sheyingshiXingjiTypesOptions != null  && sheyingshiXingjiTypesOptions.length > 0 ) {
            for (var i = 0; i < sheyingshiXingjiTypesOptions.length; i++) {
                if (sheyingshiXingjiTypesOptions[i].codeIndex == ruleForm.sheyingshiXingjiTypes) {//下拉框value对比,如果一致就赋值汉字
                        sheyingshiXingjiTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.sheyingshiContent != null && ruleForm.sheyingshiContent != 'null' && ruleForm.sheyingshiContent != '' && $("#sheyingshiContentupload").length>0) {
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#sheyingshiContentupload").length>0) {
    ...
    // 获取当前详情
    function getDetails() {
        var addsheyingshi = window.sessionStorage.getItem("addsheyingshi");
        if (addsheyingshi != null && addsheyingshi != "" && addsheyingshi != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("sheyingshi/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".aaaaaa").remove();
        }else if (window.sessionStorage.getItem('role') == '摄影师') {
            $('#sheyingshiXingjiTypesSelect').attr('readonly', 'readonly');
            $('#sheyingshiXingjiTypesSelect').attr('style', 'pointer-events: none;');
        }else{
            // alert("未知情况.......");
        }
    }
