            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">摄影师收藏展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">摄影师名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="sheyingshiName" id="sheyingshiName" placeholder="摄影师名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">摄影师手机号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="sheyingshiPhone" id="sheyingshiPhone" placeholder="摄影师手机号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">摄影师身份证号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="sheyingshiIdNumber" id="sheyingshiIdNumber" placeholder="摄影师身份证号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="ageStart" id="ageStart"  lay-skin="integer"   placeholder="年龄开始" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <div class="layui-input-inline" style="margin-top: -5px">
                                  <input type="text" name="ageEnd"   id="ageEnd"    lay-skin="integer|ageEnd"   placeholder="年龄结束" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="fenggeTypes" id="fenggeTypes" lay-filter="fenggeTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in fenggeTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="sheyingshiXingjiTypes" id="sheyingshiXingjiTypes" lay-filter="sheyingshiXingjiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in sheyingshiXingjiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../sheyingshi/detail.jsp?id='+item.sheyingshiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.sheyingshiPhoto?item.sheyingshiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../sheyingshi/detail.jsp?id='+item.sheyingshiId)"><span>{{item.sheyingshiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.sheyingshiNewMoney"><span style="color: red;">{{item.sheyingshiNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteSheyingshiCollection(item.id)">删除 {{item.sheyingshiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            sheyingshiCollectionTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 sheyingshiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            sheyingshiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteSheyingshiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('sheyingshiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        form.verify({
            ...
            ,ageEnd: function(value, item){ //value：表单的值、item：表单的DOM对象
                var ageStart = $("#ageStart").val();
                if(ageStart != null && ageStart != "" && ageStart != "null"){
                    var number = parseInt(value)- parseInt(ageStart);
                    if(number<0){
                        return '年龄的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#sheyingshiName').val() != null && jquery('#sheyingshiName').val() != "" && jquery('#sheyingshiName').val() != "null" ) {
                param['sheyingshiName'] = jquery('#sheyingshiName').val();
            }
                     
            if (jquery('#sheyingshiPhone').val() != null && jquery('#sheyingshiPhone').val() != "" && jquery('#sheyingshiPhone').val() != "null" ) {
                param['sheyingshiPhone'] = jquery('#sheyingshiPhone').val();
            }
                     
            if (jquery('#sheyingshiIdNumber').val() != null && jquery('#sheyingshiIdNumber').val() != "" && jquery('#sheyingshiIdNumber').val() != "null" ) {
                param['sheyingshiIdNumber'] = jquery('#sheyingshiIdNumber').val();
            }
                                         
            if (jquery('#ageStart').val() != null && jquery('#ageStart').val() != "" && jquery('#ageStart').val() != "null" ) {
                param['ageStart'] = jquery('#ageStart').val();
            }
            if (jquery('#ageEnd').val() != null && jquery('#ageEnd').val() != "" && jquery('#ageEnd').val() != "null" ) {
                param['ageEnd'] = jquery('#ageEnd').val();
            }
                     
            var fenggeTypes = $("#fenggeTypes").find("option:selected").val();
            if (fenggeTypes != null && fenggeTypes != "" && fenggeTypes != "null") {
                            param['fenggeTypes'] = fenggeTypes;
                        }
                     
            var sheyingshiXingjiTypes = $("#sheyingshiXingjiTypes").find("option:selected").val();
            if (sheyingshiXingjiTypes != null && sheyingshiXingjiTypes != "" && sheyingshiXingjiTypes != "null") {
                            param['sheyingshiXingjiTypes'] = sheyingshiXingjiTypes;
                        }
                                                            
            // 当前表
                                                    // 获取列表数据
            http.request('sheyingshiCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function fenggeTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fengge_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.fenggeTypesList = res.data.list;
               }
           });
       }
       ...
       function sheyingshiXingjiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=sheyingshi_xingji_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.sheyingshiXingjiTypesList = res.data.list;
               }
           });
       }
