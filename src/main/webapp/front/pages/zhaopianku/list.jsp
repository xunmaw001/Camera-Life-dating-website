            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">照片库展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">照片名字</label>
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopiankuName" id="zhaopiankuName" placeholder="照片名字" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="zhaopiankuTypes" id="zhaopiankuTypes" lay-filter="zhaopiankuTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in zhaopiankuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../zhaopianku/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.zhaopiankuPhoto?item.zhaopiankuPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../zhaopianku/detail.jsp?id='+item.id)"><span>{{item.zhaopiankuName}}</span></div>
                                <div class="ng-prslist-tit" v-if="item.zhaopiankuNewMoney"><span style="color: red;">{{item.zhaopiankuNewMoney}} RMB</span></div>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zhaopiankuContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zhaopiankuContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#zhaopiankuName').val() != null && jquery('#zhaopiankuName').val() != "" && jquery('#zhaopiankuName').val() != "null" ) {
                param['zhaopiankuName'] = jquery('#zhaopiankuName').val();
            }
         
            var zhaopiankuTypes = $("#zhaopiankuTypes").find("option:selected").val();
            if (zhaopiankuTypes != null && zhaopiankuTypes != "" && zhaopiankuTypes != "null") {
                param['zhaopiankuTypes'] = zhaopiankuTypes;
            }
                                    // 获取列表数据
            http.request('zhaopianku/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function zhaopiankuTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhaopianku_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.zhaopiankuTypesList = res.data.list;
               }
           });
       }
