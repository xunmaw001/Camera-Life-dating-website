...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的摄影师订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li v-for="item in sheyingshiOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.sheyingshiName}}</td>
                            <td style="width: 60px;">{{item.sheyingshiPhone}}</td>
                            <td style="width: 60px;">{{item.sheyingshiIdNumber}}</td>
                            <td style="width: 80px;">{{item.sexValue}}</td>
                            <td style="width: 60px;">{{item.age}}</td>
                            <td style="width: 80px;">{{item.fenggeValue}}</td>
                            <td style="width: 80px;">{{item.sheyingshiXingjiValue}}</td>
                            <td style="width: 250px;">
                                <img :src="item.sheyingshiPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.sheyingshiOrderValue}}</td>
                            <td style="width: 150px;">{{item.sheyingshiOrderStartTime}}</td>
                            <td style="width: 150px;">{{item.sheyingshiOrderEndTime}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 sheyingshiOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            sheyingshiOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(sheyingshiOrderTypes) {
                this.sheyingshiOrderTypes = sheyingshiOrderTypes;
                // 获取列表数据
                layui.http.request('sheyingshiOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    sheyingshiOrderTypes: vue.sheyingshiOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('sheyingshiOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    sheyingshiOrderTypes: this.sheyingshiOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`sheyingshiOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`sheyingshiOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('sheyingshiOrder/list', 'get',{
            page: 1,
            limit: limit,
            sheyingshiOrderTypes: vue.sheyingshiOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('sheyingshiOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            sheyingshiOrderType: vue.sheyingshiOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
       ...
       function sheyingshiOrderTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=sheyingshi_order_types", 'get', {}, function (res) {
               if(res.code == 0){
